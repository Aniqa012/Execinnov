...
export default function EditToolForm({ tool, categories }: EditToolFormProps) {
  const router = useRouter();
  const [formData, setFormData] = useState<UpdateToolData>({
    title: "",
    description: "",
    systemInstructions: "",
    questions: [{ question: "", answer: "", maxAnsLength: 600 }],
    isPro: false,
    isActive: true,
    category: "",
  });
  ...
  useEffect(() => {
    if (tool) {
      setFormData({
        ...
        systemInstructions: tool.systemInstructions,
        questions: (tool.questions && tool.questions.length > 0) ? tool.questions.map(q => ({
          question: q.question,
          answer: q.answer || "",
          maxAnsLength: q.maxAnsLength || 600
        })) : [{ question: "", answer: "", maxAnsLength: 600 }],
        isPro: tool.isPro,
  ...
  const updateToolMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(`Tool "${data.title}" updated successfully!`);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const validation = updateToolSchema.safeParse(formData);
    ...
    updateToolMutation.mutate(validation.data);
  };

  const handleInputChange = (field: keyof UpdateToolData) => (
  ...
  const addQuestion = () => {
    setFormData((prev) => ({
      ...prev,
      questions: [...prev.questions, { question: "", answer: "", maxAnsLength: 600 }],
    }));
  };
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          Update the tool details and configurations
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Update the basic details about your tool
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
                  ...
                  <CardDescription>
                    Manage questions that users will answer when using this tool.
                  </CardDescription>
                </div>
              <Button
                type="button"
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <Button
                          ...
                          disabled={updateToolMutation.isPending || formData.questions.length <= 1}
        ...
        <div className="flex justify-end space-x-4">
          ...
          <Button
            ...
            disabled={updateToolMutation.isPending}
          >
            {updateToolMutation.isPending ? "Updating Tool..." : "Update Tool"}
          </Button>
        </div>
      </form>
    </div>
